...
const EditPatient = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      ...
      try {
        ...
        setFetchError('Could not load patient data. The patient may not exist.');
      } finally {
  ...
  const validate = () => {
    ...
    if (!form.age || isNaN(form.age) || form.age < 0 || form.age > 130) e.age = 'Enter a valid age.';
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await patientService.update(id, { ...form, age: Number(form.age) });
      setSaveSuccess(true);
  ...
  if (fetchError) return (
    <div className="flex-1 p-8 h-screen overflow-y-auto">
      ...
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-slate-500 text-lg font-medium">{fetchError}</p>
  ...
  return (
    <div className="flex-1 p-8 h-screen overflow-y-auto bg-slate-50">
      <div className="mb-6">
        <Link to={`/patients/${id}`} className="inline-flex items-center text-slate-500 hover:text-primary-600 mb-4 transition-colors text-sm">
          <ArrowLeft size={16} className="mr-2" /> Back to Patient
        </Link>
        <h1 className="text-2xl font-bold text-slate-800">Edit Patient</h1>
        <p className="text-slate-500 mt-0.5 text-sm">Update the patient's medical record</p>
      </div>
      ...
      >
        {saveSuccess && (
          <div className="mb-6 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-sm font-medium flex items-center gap-2">
            <Save size={16} /> Patient updated successfully. Redirecting...
          </div>
        )}
        {errors.submit && (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              <input
                type="text" placeholder="Add a condition..."
                value={newHistory}
          ...
          {/* Allergies */}
          <div>
            <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Allergies</h2>
            <div className="flex gap-2 mb-3">
              <input
                type="text" placeholder="Add an allergy..."
                value={newAllergy}
